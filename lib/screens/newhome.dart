...
import 'package:healthcare/screens/newnew.dart';
...
import 'package:healthcare/widgets/pdfview.dart';
...
class HomePage extends StatelessWidget {
  ...
  final PDFFetchService _pdfFetchService = PDFFetchService();
  ...
  Future<void> _uploadFile(BuildContext context,String username) async {
    ...
    if (filePath != null) {
      ...
      try {
        ...
        await _firebaseFirestore.collection('pdfs').doc(username).collection(username).add({
          "name": fileName,
          "url": downloadURL,
        });
    ...
    return FutureBuilder(
      ...
      builder: (context, AsyncSnapshot<List<dynamic>> snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          String directoryPath = 'pdfs/$username';
          return Scaffold(
            body: FutureBuilder(
              future: _firebaseStorageService.fetchUserPDFs(username),
              builder: (context, AsyncSnapshot<List<String>> filesSnapshot) {
                if (filesSnapshot.connectionState == ConnectionState.waiting) {
                  return Center(child: CircularProgressIndicator());
                } else if (filesSnapshot.hasError || filesSnapshot.data == null || filesSnapshot.data!.isEmpty) {
                    return newnewPage();
                } else {
                  List<String> pdfFiles = filesSnapshot.data!;
                  return Scaffold(
                                  ...
                                  return Container(
                                    ...
                                    child: ListTile(
                                      ...
                                      onTap: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(builder: (context) =>PDFViewerScreen(pdfUrl: pdfFiles[index],),)
                                        );
                                      },
                                    ),
                                  );
